...
def compute_delta_p(Rot, p):
    ...
    k_max = int(p.shape[0] / step_size) - 1
...
def train_filter(args, dataset):
    iekf = prepare_filter(args)
    prepare_loss_data(args, dataset) # needs checking
    save_iekf(args, iekf)
...
def train_loop(args, dataset, epoch, iekf, optimizer, seq_dim):
    ...
    for i, (dataset_name, Ns) in enumerate(dataset.datasets_train_filter.items()):
        ...
        loss = mini_batch_step(iekf, dataset.list_rpe[dataset_name], t, ang_gt[0], v_gt[0], u, N0)
...
def mini_batch_step(iekf, list_rpe, t, pose0, v0, u, N0):
    iekf.set_Q()
    ...
    delta_p, delta_p_gt = precompute_lost(Rot, p, list_rpe, N0) # TODO
    if delta_p is None:
        return -1
